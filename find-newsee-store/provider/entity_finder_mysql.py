...
class EntityFinderMySQL:
    ...
    def _add_documents_to_collection(self, entity_type: str, documents: List[Dict]):
        ...
        ids = [f"{entity_type}_{i}" for i in range(len(documents))]
        ...
        logger.info(f"已添加{len(documents)}条{entity_type}数据到集合")
    ...
    def _build_document_text(self, entity_type: str, data: Dict[str, Any]) -> str:
        ...
        """
        if entity_type == 'project':
            return f"{data.get('name', '')} {data.get('location', '')} {data.get('description', '')}"
        elif entity_type == 'property':
            return f"{data.get('name', '')} {data.get('project_name', '')} {data.get('address', '')} {data.get('rooms', '')}室"
        elif entity_type == 'target':
            return f"{data.get('name', '')} {data.get('description', '')} {data.get('unit', '')}"
        return ""
    
    def search(self, query: str, entity_type: str = None, top_k: int = None) -> Dict[str, Any]:
        """
        搜索实体
    ...
    def _fuzzy_match(self, query: str, entity_type: str = None, top_k: int = 5) -> Dict[str, Any]:
        """模糊匹配"""
        results = []
        
        # 确定要搜索的集合
        collections_to_search = [entity_type] if entity_type else self.collections.keys()
    ...
    def _vector_search(self, query: str, entity_type: str = None, top_k: int = 5) -> Dict[str, Any]:
        """向量搜索"""
        results = []
    ...
    try:
        ...
        if results["found"]:
            ...
            for i, result in enumerate(results["results"], 1):
                print(f"{i}. {result['name']} ({result['type']}) - 相似度: {result['similarity']}")
        else:
            print("未找到匹配结果")
